% find jacobians and position
function [ jacobian,p ] = pos_jacobian( x,model )
...
p=(p1+p2+r3(x(1),'y')*...
    ...
    (p9+r3(x(8),'z')*...
    (p10+r3(x(9),'y')*...
    (p11+r3(-theta(1),'x')*r3(-theta(2),'y')*r3(x(10),'x')*r3(theta(2),'y')*r3(theta(1),'x')*...
    (p12+r3(-theta(3),'x')*r3(-theta(4),'y')*r3(x(11),'x')*r3(theta(4),'y')*r3(theta(3),'x')*p13)))))))))));
jacobian=zeros(3,11);
j=ones(10);n=zeros(1,11);
for i=1:11;
    j(i)=0;n(i)=1;
    jacobian(:,i)=((p1+...
        p2)*j(1)+r3(x(1),'y',n(1))*...
